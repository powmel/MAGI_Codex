...
| --- | --- | --- |
| `Dependency checks failed` | Missing ffmpeg or STT backend | `.\scripts\doctor.ps1 -ConfigPath .\configs\pipeline-settings.json` |
| `ffmpeg not found` | ffmpeg not installed or missing PATH | `winget install Gyan.FFmpeg` |
| `No whisper backend found` | whisper.cpp/faster-whisper not installed | `pip install faster-whisper` or install whisper.cpp |
| `Smoke test failed` | Missing outputs or WAV processing errors | `.\scripts\smoke_test.ps1` and review logs in `F:\MAGI\logs` |
